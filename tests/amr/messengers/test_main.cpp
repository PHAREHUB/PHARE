...
TEST_F(AfullHybridBasicHierarchy, initializesFieldsOnRefinedLevels)
{
    ...
    for (auto iLevel = 0; iLevel < hierarchy.getNumberOfLevels(); ++iLevel)
    {
        ...
        for (auto& patch : *level)
        {
            ...
            auto& Vix = hybridModel->state.ions.velocity().getComponent(PHARE::Component::X);
            auto& Viy = hybridModel->state.ions.velocity().getComponent(PHARE::Component::Y);
            auto& Viz = hybridModel->state.ions.velocity().getComponent(PHARE::Component::Z);
