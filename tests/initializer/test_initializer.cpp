...
TEST(APythonDataProvider, providesAValidTree)
{
    ...
    auto dt             = input["simulation"]["time_step"].to<floater_t<4>>();

    auto layout = input["simulation"]["grid"]["layout_type"].to<std::string>();
    auto nx     = input["simulation"]["grid"]["nbr_cells"]["x"].to<int>();
    auto dx     = input["simulation"]["grid"]["meshsize"]["x"].to<floater_t<4>>();
    auto origin = input["simulation"]["grid"]["origin"]["x"].to<floater_t<4>>();
    ...
    auto pop0Mass                 = pop0["mass"].to<floater_t<4>>();
    ...
    auto pop0Charge         = pop0ParticleInitializer["charge"].to<floater_t<4>>();
    ...
    EXPECT_EQ(1, dim);
    EXPECT_EQ(65, nx);
    EXPECT_FLOAT_EQ(1. / 65., dx);
    EXPECT_FLOAT_EQ(0.001, dt);
    EXPECT_FLOAT_EQ(0., origin);
    EXPECT_EQ("yee", layout);

    EXPECT_EQ("modified_boris", pusherName);

    std::vector<floater_t<4>> input_2 = {2};

    EXPECT_EQ(2, nbrPopulations);
    EXPECT_EQ("protons", pop0Name);
    EXPECT_FLOAT_EQ(1., pop0Mass);
    EXPECT_EQ("maxwellian", pop0ParticleInitializerName);
    EXPECT_FLOAT_EQ(2., (*pop0density(input_2))[0]);
    EXPECT_FLOAT_EQ(1., (*bulk0x(input_2))[0]);
    EXPECT_FLOAT_EQ(1., (*bulk0y(input_2))[0]);
    EXPECT_FLOAT_EQ(1., (*bulk0z(input_2))[0]);
    EXPECT_FLOAT_EQ(1., (*vth0x(input_2))[0]);
    EXPECT_FLOAT_EQ(1., (*vth0y(input_2))[0]);
    EXPECT_FLOAT_EQ(1., (*vth0z(input_2))[0]);
    EXPECT_EQ(100, pop0NbrPartPerCell);
    EXPECT_FLOAT_EQ(1., pop0Charge);
