...
// ---------------------------------------------------------------------------
// Scalar tests (falls back to Dirichlet with zero value)
// ---------------------------------------------------------------------------

/**
 * @brief Ghost scalar cells are set to zero (Dirichlet fallback with value 0).
 *
 * The scalar path falls back to Dirichlet with value 0, which sets the ghost
 * cell such that linear interpolation at the boundary gives zero.
 */
TEST(FieldAntisymmetricInnerBoundaryCondition, scalarGhostCellSetToZeroOnBoundary)
{
    AntisymmetricBCFixture fix;
    ...
    for (auto const& g : ghostCells)
    {
        ...
        foundInPatch = true;
        // Dirichlet with value 0: field_ghost = 2*0 - field_mirror
        double const mirror_value = g.mirrorPoint[0] + g.mirrorPoint[1];
        double const expected      = -mirror_value;
...
/**
 * @brief For a constant vector field, antisymmetric BC reverses the tangential
 *        components and preserves the normal component.
 *
 * For V = (Cx, Cy, Cz) and normal n=(1,0,0):
 *   V_n = (Cx, 0, 0),  V_t = (0, Cy, Cz)
 *   V_antisym = V_n - V_t = (Cx, -Cy, -Cz)
 */
TEST(FieldAntisymmetricInnerBoundaryCondition, vectorConstantField_tangentialComponentsReversed)
{
    AntisymmetricBCFixture fix;
    auto const& layout   = fix.layout;
    auto const& meshData = fix.buffers.tags;
    ...
    for (auto const& g : ghostCells)
    {
        ...
        EXPECT_NEAR(Vx(g.index), Cx, eps)
            << "Vx (normal) ghost at (" << g.index[0] << "," << g.index[1] << ")";
        EXPECT_NEAR(Vy(g.index), -Cy, eps)
            << "Vy (tangential) ghost at (" << g.index[0] << "," << g.index[1] << ")";
        EXPECT_NEAR(Vz(g.index), -Cz, eps)
            << "Vz (tangential) ghost at (" << g.index[0] << "," << g.index[1] << ")";
    }
    EXPECT_TRUE(foundInPatch) << "at least one in-patch ghost must exist";
}

/**
 * @brief For a purely tangential vector field, antisymmetric BC sets all
 *        components to zero (since there's no normal component).
 *
 * If V = (0, Vy, Vz) and n=(1,0,0):
 *   V_n = (0, 0, 0),  V_t = (0, Vy, Vz)
 *   V_antisym = V_n - V_t = (0, -Vy, -Vz)
 */
TEST(FieldAntisymmetricInnerBoundaryCondition, vectorPurelyTangentialField_negated)
{
    AntisymmetricBCFixture fix;
    auto const& layout   = fix.layout;
    auto const& meshData = fix.buffers.tags;
...
/**
 * @brief For a linear vector field, the ghost receives the correctly
 *        transformed interpolated mirror value.
 *
 ...
 *   V_n      = (1.5 + y_m,  0,          0)
 *   V_t      = (0,           1.5 - y_m,  1)
 *   V_anti   = (1.5 + y_m, -(1.5-y_m), -1) = (1.5+y_m, -1.5+y_m, -1)
 */
TEST(FieldAntisymmetricInnerBoundaryCondition, vectorLinearField_correctTransformation)
{
    AntisymmetricBCFixture fix;
    auto const& layout   = fix.layout;
    auto const& meshData = fix.buffers.tags;
 ...
 *
 * Uses MHDQuantity::Vector::B in 2D Yee MHD where centerings differ per component:
 *   Bx: (primal, dual)   By: (dual, primal)   Bz: (dual, dual)
 *
 * The dual-margin reserves one extra cell of slack per dual direction. So there can exist
 * mirror points interpolable for one centering combination but not for another. The fix in
 * field_antisymmetric_inner_boundary_condition.hpp guards the per-component loop by
 * re-checking pointIsInterpolable for every sibling centering before touching the field.
 */
TEST(FieldAntisymmetricInnerBoundaryCondition, vectorMixedCenterings_skipGhostWhenSiblingNotInterpolable)
{
    ...
    // Locate ghosts (per component-i list) whose mirror is interpolable for i but not
    // for at least one sibling j != i.
    struct Trigger
    {
        std::size_t i;
    ...
    // Sentinel-fill every component at every trigger index. After bc.apply the sentinel
    // MUST remain — the new guard skips those ghosts.
