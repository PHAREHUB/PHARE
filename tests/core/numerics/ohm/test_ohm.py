...
def test_ohm_yee1D(path):
    ...
    # ideal term
    idealx[psi_d_X : pei_d_X + 1] = (
        0.5
        * (Vz[psi_p_X:pei_p_X] + Vz[psi_p_X + 1 : pei_p_X + 1])
        * By[psi_d_X : pei_d_X + 1]
        - 0.5
        * (Vy[psi_p_X:pei_p_X] + Vy[psi_p_X + 1 : pei_p_X + 1])
        * Bz[psi_d_X : pei_d_X + 1]
    )

    idealy[psi_p_X : pei_p_X + 1] = (
        0.5
        * Vx[psi_p_X : pei_p_X + 1]
        * (Bz[psi_d_X - 1 : pei_d_X + 1] + Bz[psi_d_X : pei_d_X + 2])
        - Vz[psi_p_X : pei_p_X + 1] * Bx[psi_p_X : pei_p_X + 1]
    )

    idealz[psi_p_X : pei_p_X + 1] = Vy[psi_p_X : pei_p_X + 1] * Bx[
        psi_p_X : pei_p_X + 1
    ] - 0.5 * Vx[psi_p_X : pei_p_X + 1] * (
        By[psi_d_X - 1 : pei_d_X + 1] + By[psi_d_X : pei_d_X + 2]
    )

    # pressure term
    pressx[psi_d_X : pei_d_X + 1] = -(
        (P[psi_p_X + 1 : pei_p_X + 1] - P[psi_p_X:pei_p_X]) / tv.meshSize[0]
...
def test_ohm_yee2D(path):
    ...
    # analytical profiles of density, velocity...
    n = np.tensordot(np.cosh(0.5 * x_primal), np.cosh(0.5 * y_primal), axes=0)
    Vx = np.tensordot(np.sinh(0.2 * x_primal), np.sinh(0.2 * y_primal), axes=0)
    Vy = np.tensordot(np.sinh(0.3 * x_primal), np.sinh(0.3 * y_primal), axes=0)
    Vz = np.tensordot(np.sinh(0.4 * x_primal), np.sinh(0.4 * y_primal), axes=0)
    P = np.tensordot(np.cosh(0.5 * x_primal), np.cosh(0.5 * y_primal), axes=0)
    Bx = np.tensordot(np.cosh(0.2 * x_primal), np.cosh(0.2 * y_dual), axes=0)
    By = np.tensordot(np.cosh(0.3 * x_dual), np.cosh(0.3 * y_primal), axes=0)
    Bz = np.tensordot(np.cosh(0.4 * x_dual), np.cosh(0.4 * y_dual), axes=0)
    Jx = np.tensordot(np.tanh(0.2 * x_dual), np.tanh(0.2 * y_primal), axes=0)
    Jy = np.tensordot(np.tanh(0.3 * x_primal), np.tanh(0.3 * y_dual), axes=0)
    Jz = np.tensordot(np.tanh(0.4 * x_primal), np.tanh(0.4 * y_primal), axes=0)

    # ideal term
    idealx[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1] = (
        -0.25
        * (
            Vy[psi_p_X:pei_p_X, psi_p_Y : pei_p_Y + 1]
        ...
        * (
        ...
        + 0.5
        * (
            Vz[psi_p_X:pei_p_X, psi_p_Y : pei_p_Y + 1]
            + Vz[psi_p_X + 1 : pei_p_X + 1, psi_p_Y : pei_p_Y + 1]
        )
        * By[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1]
    )

    idealy[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1] = -0.5 * (
        Vz[psi_p_X : pei_p_X + 1, psi_p_Y:pei_p_Y]
        + Vz[psi_p_X : pei_p_X + 1, psi_p_Y + 1 : pei_p_Y + 1]
    ) * (Bx[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1]) + 0.25 * (
        Vx[psi_p_X : pei_p_X + 1, psi_p_Y:pei_p_Y]
    ...
    idealz[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1] = -0.5 * Vx[
        psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1
    ] * (
    ...
    pressx[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1] = -(
        (
            ...
            - P[psi_p_X:pei_p_X, psi_p_Y : pei_p_Y + 1]
        )
        / tv.meshSize[0]
    ) / (
        0.5
        * (
            n[psi_p_X:pei_p_X, psi_p_Y : pei_p_Y + 1]
    ...
    pressy[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1] = -(
        (
            ...
            - P[psi_p_X : pei_p_X + 1, psi_p_Y:pei_p_Y]
        )
        / tv.meshSize[1]
    ) / (
        0.5
        * (
            n[psi_p_X : pei_p_X + 1, psi_p_Y:pei_p_Y]
    ...
    # viscous term
    viscousx[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1] = -nu * (
        Jx[psi_d_X - 1 : pei_d_X, psi_p_Y : pei_p_Y + 1]
        - 2.0 * Jx[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1]
        + Jx[psi_d_X + 1 : pei_d_X + 2, psi_p_Y : pei_p_Y + 1]
    ) / (tv.meshSize[0] * tv.meshSize[0]) - nu * (
        Jx[psi_d_X : pei_d_X + 1, psi_p_Y - 1 : pei_p_Y]
        - 2.0 * Jx[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1]
        + Jx[psi_d_X : pei_d_X + 1, psi_p_Y + 1 : pei_p_Y + 2]
    ) / (
        tv.meshSize[1] * tv.meshSize[1]
    )
    viscousy[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1] = -nu * (
        Jy[psi_p_X - 1 : pei_p_X, psi_d_Y : pei_d_Y + 1]
        - 2.0 * Jy[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1]
        + Jy[psi_p_X + 1 : pei_p_X + 2, psi_d_Y : pei_d_Y + 1]
    ) / (tv.meshSize[0] * tv.meshSize[0]) - nu * (
        Jy[psi_p_X : pei_p_X + 1, psi_d_Y - 1 : pei_d_Y]
        - 2.0 * Jy[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1]
        + Jy[psi_p_X : pei_p_X + 1, psi_d_Y + 1 : pei_d_Y + 2]
    ) / (
        tv.meshSize[1] * tv.meshSize[1]
    )
    viscousz[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1] = -nu * (
        Jz[psi_p_X - 1 : pei_p_X, psi_p_Y : pei_p_Y + 1]
        - 2.0 * Jz[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1]
        + Jz[psi_p_X + 1 : pei_p_X + 2, psi_p_Y : pei_p_Y + 1]
    ) / (tv.meshSize[0] * tv.meshSize[0]) - nu * (
        Jz[psi_p_X : pei_p_X + 1, psi_p_Y - 1 : pei_p_Y]
        - 2.0 * Jz[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1]
...
def test_ohm_yee3D(path):
    ...
    # analytical profiles of density, velocity...
    n = np.tensordot(
        np.cosh(0.5 * x_primal),
        np.tensordot(np.cosh(0.5 * y_primal), np.cosh(0.5 * z_primal), axes=0),
        axes=0,
    )
    Vx = np.tensordot(
        np.sinh(0.2 * x_primal),
        np.tensordot(np.sinh(0.2 * y_primal), np.sinh(0.2 * z_primal), axes=0),
        axes=0,
    )
    Vy = np.tensordot(
        np.sinh(0.3 * x_primal),
        np.tensordot(np.sinh(0.3 * y_primal), np.sinh(0.3 * z_primal), axes=0),
        axes=0,
    )
    Vz = np.tensordot(
        np.sinh(0.4 * x_primal),
        np.tensordot(np.sinh(0.4 * y_primal), np.sinh(0.4 * z_primal), axes=0),
        axes=0,
    )
    P = np.tensordot(
        np.cosh(0.5 * x_primal),
    ...
    Bx = np.tensordot(
        np.cosh(0.2 * x_primal),
        np.tensordot(np.cosh(0.2 * y_dual), np.cosh(0.2 * z_dual), axes=0),
        axes=0,
    )
    By = np.tensordot(
        np.cosh(0.3 * x_dual),
        np.tensordot(np.cosh(0.3 * y_primal), np.cosh(0.3 * z_dual), axes=0),
        axes=0,
    )
    Bz = np.tensordot(
        np.cosh(0.4 * x_dual),
        np.tensordot(np.cosh(0.4 * y_dual), np.cosh(0.4 * z_primal), axes=0),
        axes=0,
    )
    Jx = np.tensordot(
        np.tanh(0.2 * x_dual),
        np.tensordot(np.tanh(0.2 * y_primal), np.tanh(0.2 * z_primal), axes=0),
        axes=0,
    )
    Jy = np.tensordot(
        np.tanh(0.3 * x_primal),
        np.tensordot(np.tanh(0.3 * y_dual), np.tanh(0.3 * z_primal), axes=0),
        axes=0,
    )
    Jz = np.tensordot(
        np.tanh(0.4 * x_primal),
        np.tensordot(np.tanh(0.4 * y_primal), np.tanh(0.4 * z_dual), axes=0),
        axes=0,
    )

    # ideal term
    idealx[
        psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1
    ] = -0.25 * (
        Vy[psi_p_X:pei_p_X, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1]
        ...
        Vz[psi_p_X:pei_p_X, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1]
    ...
    idealy[
        psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1, psi_p_Z : pei_p_Z + 1
    ] = -0.25 * (
        Vz[psi_p_X : pei_p_X + 1, psi_p_Y:pei_p_Y, psi_p_Z : pei_p_Z + 1]
        ...
        Vx[psi_p_X : pei_p_X + 1, psi_p_Y:pei_p_Y, psi_p_Z : pei_p_Z + 1]
    ...
    idealz[
        psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_d_Z : pei_d_Z + 1
    ] = -0.25 * (
        Vx[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z:pei_p_Z]
        ...
        Vy[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z:pei_p_Z]
    ...
    pressx[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1] = -(
        (
            ...
            - P[psi_p_X:pei_p_X, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1]
        )
        / tv.meshSize[0]
    ) / (
        0.5
        * (
            n[psi_p_X:pei_p_X, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1]
    ...
    pressy[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1, psi_p_Z : pei_p_Z + 1] = -(
        (
            ...
            - P[psi_p_X : pei_p_X + 1, psi_p_Y:pei_p_Y, psi_p_Z : pei_p_Z + 1]
        )
        / tv.meshSize[1]
    ) / (
        0.5
        * (
            n[psi_p_X : pei_p_X + 1, psi_p_Y:pei_p_Y, psi_p_Z : pei_p_Z + 1]
    ...
    pressz[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_d_Z : pei_d_Z + 1] = -(
        (
            ...
            - P[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z:pei_p_Z]
        )
        / tv.meshSize[2]
    ) / (
        0.5
        * (
            n[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z:pei_p_Z]
    ...
    viscousx[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1] = (
        -nu
        * (
            Jx[psi_d_X - 1 : pei_d_X, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1]
            - 2.0
            * Jx[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1]
            + Jx[
                psi_d_X + 1 : pei_d_X + 2, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1
            ]
        )
        / (tv.meshSize[0] * tv.meshSize[0])
        - nu
        * (
            Jx[psi_d_X : pei_d_X + 1, psi_p_Y - 1 : pei_p_Y, psi_p_Z : pei_p_Z + 1]
            - 2.0
            * Jx[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1]
            + Jx[
                psi_d_X : pei_d_X + 1, psi_p_Y + 1 : pei_p_Y + 2, psi_p_Z : pei_p_Z + 1
            ]
        )
        / (tv.meshSize[1] * tv.meshSize[1])
        - nu
        * (
            Jx[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z - 1 : pei_p_Z]
            - 2.0
            * Jx[psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z : pei_p_Z + 1]
            + Jx[
                psi_d_X : pei_d_X + 1, psi_p_Y : pei_p_Y + 1, psi_p_Z + 1 : pei_p_Z + 2
            ]
        )
    ...
    viscousy[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1, psi_p_Z : pei_p_Z + 1] = (
        -nu
        * (
            Jy[psi_p_X - 1 : pei_p_X, psi_d_Y : pei_d_Y + 1, psi_p_Z : pei_p_Z + 1]
            - 2.0
            * Jy[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1, psi_p_Z : pei_p_Z + 1]
            + Jy[
                psi_p_X + 1 : pei_p_X + 2, psi_d_Y : pei_d_Y + 1, psi_p_Z : pei_p_Z + 1
            ]
        )
        / (tv.meshSize[0] * tv.meshSize[0])
        - nu
        * (
            Jy[psi_p_X : pei_p_X + 1, psi_d_Y - 1 : pei_d_Y, psi_p_Z : pei_p_Z + 1]
            - 2.0
            * Jy[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1, psi_p_Z : pei_p_Z + 1]
            + Jy[
                psi_p_X : pei_p_X + 1, psi_d_Y + 1 : pei_d_Y + 2, psi_p_Z : pei_p_Z + 1
            ]
        )
        / (tv.meshSize[1] * tv.meshSize[1])
        - nu
        * (
            Jy[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1, psi_p_Z - 1 : pei_p_Z]
            - 2.0
            * Jy[psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1, psi_p_Z : pei_p_Z + 1]
            + Jy[
                psi_p_X : pei_p_X + 1, psi_d_Y : pei_d_Y + 1, psi_p_Z + 1 : pei_p_Z + 2
            ]
        )
        / (tv.meshSize[2] * tv.meshSize[2])
    )
    ...
    viscousz[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_d_Z : pei_d_Z + 1] = (
        -nu
        * (
            Jz[psi_p_X - 1 : pei_p_X, psi_p_Y : pei_p_Y + 1, psi_d_Z : pei_d_Z + 1]
            - 2.0
            * Jz[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_d_Z : pei_d_Z + 1]
            + Jz[
                psi_p_X + 1 : pei_p_X + 2, psi_p_Y : pei_p_Y + 1, psi_d_Z : pei_d_Z + 1
            ]
        )
        / (tv.meshSize[0] * tv.meshSize[0])
        - nu
        * (
            Jz[psi_p_X : pei_p_X + 1, psi_p_Y - 1 : pei_p_Y, psi_d_Z : pei_d_Z + 1]
            - 2.0
            * Jz[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_d_Z : pei_d_Z + 1]
            + Jz[
                psi_p_X : pei_p_X + 1, psi_p_Y + 1 : pei_p_Y + 2, psi_d_Z : pei_d_Z + 1
            ]
        )
        / (tv.meshSize[1] * tv.meshSize[1])
        - nu
        * (
            Jz[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_d_Z - 1 : pei_d_Z]
            - 2.0
            * Jz[psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_d_Z : pei_d_Z + 1]
            + Jz[
                psi_p_X : pei_p_X + 1, psi_p_Y : pei_p_Y + 1, psi_d_Z + 1 : pei_d_Z + 2
            ]
        )
        / (tv.meshSize[2] * tv.meshSize[2])
    )
