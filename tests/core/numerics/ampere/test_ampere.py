...
def test_ampere_yee1D(path):
    ...
    Jy[psi_p_X : pei_p_X + 1] = (
        -(Bz[psi_d_X : pei_d_X + 2] - Bz[psi_d_X - 1 : pei_d_X + 1]) / tv.meshSize[0]
    )
...
def test_ampere_yee3D(path):
    ...
    Bx = np.tensordot(
        ...
        np.tensordot(
            np.cos(2 * np.pi / tv.domainSize[1] * y_dual),
            np.tanh(2 * np.pi / tv.domainSize[2] * z_dual),
            axes=0,
        ),
        axes=0,
    )
    By = np.tensordot(
        np.tanh(2 * np.pi / tv.domainSize[0] * x_dual),
        np.tensordot(
            np.sin(2 * np.pi / tv.domainSize[1] * y_primal),
            np.cos(2 * np.pi / tv.domainSize[2] * z_dual),
            axes=0,
        ),
        axes=0,
    )
    Bz = np.tensordot(
        ...
        np.tensordot(
            np.tanh(2 * np.pi / tv.domainSize[1] * y_dual),
            np.sin(2 * np.pi / tv.domainSize[2] * z_primal),
            axes=0,
        ),
        axes=0,
    )
