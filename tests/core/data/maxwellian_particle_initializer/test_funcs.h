#ifndef PHARE_TEST_MAXWELLIAN_PARTICLE_INITIALIZER_TEST_FUNCS_H
#define PHARE_TEST_MAXWELLIAN_PARTICLE_INITIALIZER_TEST_FUNCS_H


#include <memory>
#include <vector>


namespace PHARE::core::test_fn::func_1d
{
double density(double x)
{
    return 1;
}

double vx(double x)
{
    return 1;
}

double vy(double x)
{
    return 1;
}

double vz(double x)
{
    return 1;
}

double vthx(double x)
{
    return 1;
}

double vthy(double x)
{
    return 1;
}

double vthz(double x)
{
    return 1;
}

double bx(double x)
{
    return 1;
}

double by(double x)
{
    return 1;
}

double bz(double x)
{
    return 1;
}

} // namespace PHARE::core::test_fn::func_1d


namespace PHARE::core::test_fn::func_2d
{
double density(double x, double y)
{
    return 1;
}

double vx(double x, double y)
{
    return 1;
}

double vy(double x, double y)
{
    return 1;
}

double vz(double x, double y)
{
    return 1;
}

double vthx(double x, double y)
{
    return 1;
}

double vthy(double x, double y)
{
    return 1;
}

double vthz(double x, double y)
{
    return 1;
}

double bx(double x, double y)
{
    return 1;
}

double by(double x, double y)
{
    return 1;
}

double bz(double x, double y)
{
    return 1;
}

} // namespace PHARE::core::test_fn::func_2d


namespace PHARE::core::test_fn::func_3d
{
double density(double x, double y, double z)
{
    return 1;
}

double vx(double x, double y, double z)
{
    return 1;
}

double vy(double x, double y, double z)
{
    return 1;
}

double vz(double x, double y, double z)
{
    return 1;
}

double vthx(double x, double y, double z)
{
    return 1;
}

double vthy(double x, double y, double z)
{
    return 1;
}

double vthz(double x, double y, double z)
{
    return 1;
}

double bx(double x, double y, double z)
{
    return 1;
}

double by(double x, double y, double z)
{
    return 1;
}

double bz(double x, double y, double z)
{
    return 1;
}


} // namespace PHARE::core::test_fn::func_3d



#endif // PHARE_TEST_MAXWELLIAN_PARTICLE_INITIALIZER_TEST_FUNCS_H
