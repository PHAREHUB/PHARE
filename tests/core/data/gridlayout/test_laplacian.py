...
def test_laplacian_yee1D(path):
    ...
    for interpOrder in tv.interpOrders:
        ...
        Jx = np.zeros(layout.allocSize(interpOrder, tv.JxCentering[0], tv.nbrCells[0]), dtype=np.float64)
        Jy = np.zeros(layout.allocSize(interpOrder, tv.JyCentering[0], tv.nbrCells[0]), dtype=np.float64)
        Jz = np.zeros(layout.allocSize(interpOrder, tv.JzCentering[0], tv.nbrCells[0]), dtype=np.float64)

        lapJx = np.zeros(layout.allocSize(interpOrder, tv.JxCentering[0], tv.nbrCells[0]), dtype=np.float64)
        lapJy = np.zeros(layout.allocSize(interpOrder, tv.JyCentering[0], tv.nbrCells[0]), dtype=np.float64)
        lapJz = np.zeros(layout.allocSize(interpOrder, tv.JzCentering[0], tv.nbrCells[0]), dtype=np.float64)
        ...
        lapJx[psi_d_X:pei_d_X+1] = (Jx[psi_d_X+1:pei_d_X+2]-2.0*Jx[psi_d_X:pei_d_X+1]+Jx[psi_d_X-1:pei_d_X])/(tv.meshSize[0]*tv.meshSize[0])
        lapJy[psi_p_X:pei_p_X+1] = (Jy[psi_p_X+1:pei_p_X+2]-2.0*Jy[psi_p_X:pei_p_X+1]+Jy[psi_p_X-1:pei_p_X])/(tv.meshSize[0]*tv.meshSize[0])
        lapJz[psi_p_X:pei_p_X+1] = (Jz[psi_p_X+1:pei_p_X+2]-2.0*Jz[psi_p_X:pei_p_X+1]+Jz[psi_p_X-1:pei_p_X])/(tv.meshSize[0]*tv.meshSize[0])
...
def test_laplacian_yee2D(path):
    ...
    for interpOrder in tv.interpOrders:
        ...
        Jx = np.zeros([layout.allocSize(interpOrder, tv.JxCentering[0], tv.nbrCells[0]),
                       layout.allocSize(interpOrder, tv.JxCentering[1], tv.nbrCells[1])], dtype=np.float64)
        Jy = np.zeros([layout.allocSize(interpOrder, tv.JyCentering[0], tv.nbrCells[0]),
                       layout.allocSize(interpOrder, tv.JyCentering[1], tv.nbrCells[1])], dtype=np.float64)
        Jz = np.zeros([layout.allocSize(interpOrder, tv.JzCentering[0], tv.nbrCells[0]),
                       layout.allocSize(interpOrder, tv.JzCentering[1], tv.nbrCells[1])], dtype=np.float64)
        ...
        lapJx = np.zeros([layout.allocSize(interpOrder, tv.JxCentering[0], tv.nbrCells[0]),
                          layout.allocSize(interpOrder, tv.JxCentering[1], tv.nbrCells[1])], dtype=np.float64)
        lapJy = np.zeros([layout.allocSize(interpOrder, tv.JyCentering[0], tv.nbrCells[0]),
                          layout.allocSize(interpOrder, tv.JyCentering[1], tv.nbrCells[1])], dtype=np.float64)
        lapJz = np.zeros([layout.allocSize(interpOrder, tv.JzCentering[0], tv.nbrCells[0]),
                          layout.allocSize(interpOrder, tv.JzCentering[1], tv.nbrCells[1])], dtype=np.float64)
        ...
        Jx = np.tensordot(np.sinh(0.1*x_dual),
                          np.cosh(0.1*y_primal), axes=0)
        Jy = np.tensordot(np.sinh(0.3*x_primal),
                          np.cosh(0.3*y_dual), axes=0)
        Jz = np.tensordot(np.sinh(0.2*x_primal),
                          np.cosh(0.2*y_primal), axes=0)

        Jx_x[psi_d_X:pei_d_X+1,:] = (Jx[psi_d_X+1:pei_d_X+2:,]-2.0*Jx[psi_d_X:pei_d_X+1,:]+Jx[psi_d_X-1:pei_d_X,:])/(tv.meshSize[0]*tv.meshSize[0])
        Jx_y[:,psi_p_Y:pei_p_Y+1] = (Jx[:,psi_p_Y+1:pei_p_Y+2]-2.0*Jx[:,psi_p_Y:pei_p_Y+1]+Jx[:,psi_p_Y-1:pei_p_Y])/(tv.meshSize[1]*tv.meshSize[1])

        Jy_x[psi_p_X:pei_p_X+1,:] = (Jy[psi_p_X+1:pei_p_X+2,:]-2.0*Jy[psi_p_X:pei_p_X+1,:]+Jy[psi_p_X-1:pei_p_X,:])/(tv.meshSize[0]*tv.meshSize[0])
        Jy_y[:,psi_d_Y:pei_d_Y+1] = (Jy[:,psi_d_Y+1:pei_d_Y+2]-2.0*Jy[:,psi_d_Y:pei_d_Y+1]+Jy[:,psi_d_Y-1:pei_d_Y])/(tv.meshSize[1]*tv.meshSize[1])

        Jz_x[psi_p_X:pei_p_X+1,:] = (Jz[psi_p_X+1:pei_p_X+2,:]-2.0*Jz[psi_p_X:pei_p_X+1,:]+Jz[psi_p_X-1:pei_p_X,:])/(tv.meshSize[0]*tv.meshSize[0])
        Jz_y[:,psi_p_Y:pei_p_Y+1] = (Jz[:,psi_p_Y+1:pei_p_Y+2]-2.0*Jz[:,psi_p_Y:pei_p_Y+1]+Jz[:,psi_p_Y-1:pei_p_Y])/(tv.meshSize[1]*tv.meshSize[1])
...
def test_laplacian_yee3D(path):
    ...
    for interpOrder in tv.interpOrders:
        ...
        Jx = np.zeros([layout.allocSize(interpOrder, tv.JxCentering[0], tv.nbrCells[0]),
                       ...
                       layout.allocSize(interpOrder, tv.JxCentering[2], tv.nbrCells[2])], dtype=np.float64)
        Jy = np.zeros([layout.allocSize(interpOrder, tv.JyCentering[0], tv.nbrCells[0]),
                       layout.allocSize(interpOrder, tv.JyCentering[1], tv.nbrCells[1]),
                       layout.allocSize(interpOrder, tv.JyCentering[2], tv.nbrCells[2])], dtype=np.float64)
        Jz = np.zeros([layout.allocSize(interpOrder, tv.JzCentering[0], tv.nbrCells[0]),
                       layout.allocSize(interpOrder, tv.JzCentering[1], tv.nbrCells[1]),
                       layout.allocSize(interpOrder, tv.JzCentering[2], tv.nbrCells[2])], dtype=np.float64)
        Jx_x = np.zeros_like(Jx)
        Jx_y = np.zeros_like(Jx)
        ...
        Jx_x[psi_d_X:pei_d_X+1,:,:] = (Jx[psi_d_X+1:pei_d_X+2,:,:]-2.0*Jx[psi_d_X:pei_d_X+1,:,:]+Jx[psi_d_X-1:pei_d_X,:,:])/(tv.meshSize[0]*tv.meshSize[0])
        Jx_y[:,psi_p_Y:pei_p_Y+1,:] = (Jx[:,psi_p_Y+1:pei_p_Y+2,:]-2.0*Jx[:,psi_p_Y:pei_p_Y+1,:]+Jx[:,psi_p_Y-1:pei_p_Y,:])/(tv.meshSize[1]*tv.meshSize[1])
        Jx_z[:,:,psi_p_Z:pei_p_Z+1] = (Jx[:,:,psi_p_Z+1:pei_p_Z+2]-2.0*Jx[:,:,psi_p_Z:pei_p_Z+1]+Jx[:,:,psi_p_Z-1:pei_p_Z])/(tv.meshSize[2]*tv.meshSize[2])

        Jy_x[psi_p_X:pei_p_X+1,:,:] = (Jy[psi_p_X+1:pei_p_X+2,:,:]-2.0*Jy[psi_p_X:pei_p_X+1,:,:]+Jy[psi_p_X-1:pei_p_X,:,:])/(tv.meshSize[0]*tv.meshSize[0])
        Jy_y[:,psi_d_Y:pei_d_Y+1,:] = (Jy[:,psi_d_Y+1:pei_d_Y+2,:]-2.0*Jy[:,psi_d_Y:pei_d_Y+1,:]+Jy[:,psi_d_Y-1:pei_d_Y,:])/(tv.meshSize[1]*tv.meshSize[1])
        Jy_z[:,:,psi_p_Z:pei_p_Z+1] = (Jy[:,:,psi_p_Z+1:pei_p_Z+2]-2.0*Jy[:,:,psi_p_Z:pei_p_Z+1]+Jy[:,:,psi_p_Z-1:pei_p_Z])/(tv.meshSize[2]*tv.meshSize[2])

        Jz_x[psi_p_X:pei_p_X+1,:,:] = (Jz[psi_p_X+1:pei_p_X+2,:,:]-2.0*Jz[psi_p_X:pei_p_X+1,:,:]+Jz[psi_p_X-1:pei_p_X,:,:])/(tv.meshSize[0]*tv.meshSize[0])
        Jz_y[:,psi_p_Y:pei_p_Y+1,:] = (Jz[:,psi_p_Y+1:pei_p_Y+2,:]-2.0*Jz[:,psi_p_Y:pei_p_Y+1,:]+Jz[:,psi_p_Y-1:pei_p_Y,:])/(tv.meshSize[1]*tv.meshSize[1])
        Jz_z[:,:,psi_d_Z:pei_d_Z+1] = (Jz[:,:,psi_d_Z+1:pei_d_Z+2]-2.0*Jz[:,:,psi_d_Z:pei_d_Z+1]+Jz[:,:,psi_d_Z-1:pei_d_Z])/(tv.meshSize[2]*tv.meshSize[2])
        ...
        np.savetxt(os.path.join(path, filename_lapJx), lapJx.flatten('C'), delimiter=" ")
        np.savetxt(os.path.join(path, filename_lapJy), lapJy.flatten('C'), delimiter=" ")
        np.savetxt(os.path.join(path, filename_lapJz), lapJz.flatten('C'), delimiter=" ")
