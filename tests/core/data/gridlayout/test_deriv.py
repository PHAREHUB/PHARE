...
def test_deriv1D(path):
    ...
    for interpOrder in tv.interpOrders:
        ...
        dxBy = np.zeros(
            layout.allocSizeDerived(interpOrder, tv.ByCentering[0], tv.nbrCells[0]),
            dtype=np.float64,
        )
        dxEz = np.zeros(
            layout.allocSizeDerived(interpOrder, tv.EzCentering[0], tv.nbrCells[0]),
            dtype=np.float64,
        )
        ...
        dxBy[psi_p_X : pei_p_X + 1] = (
            By[psi_d_X : pei_d_X + 2] - By[psi_d_X - 1 : pei_d_X + 1]
        ) / tv.meshSize[0]
        dxEz[psi_d_X : pei_d_X + 1] = (
...
def test_deriv2D(path):
    ...
    for interpOrder in tv.interpOrders:
        ...
        dyBy[:, psi_d_Y : pei_d_Y + 1] = (
            By[:, psi_p_Y + 1 : pei_p_Y + 1] - By[:, psi_p_Y:pei_p_Y]
        ) / tv.meshSize[1]
        dyEz[:, psi_d_Y : pei_d_Y + 1] = (
    ...
    for interpOrder in tv.interpOrders:
        ...
        By = np.zeros(
            [
                layout.allocSize(interpOrder, tv.ByCentering[0], tv.nbrCells[0]),
                layout.allocSize(interpOrder, tv.ByCentering[1], tv.nbrCells[1]),
                layout.allocSize(interpOrder, tv.ByCentering[2], tv.nbrCells[2]),
            ],
            dtype=np.float64,
        )
        Ez = np.zeros(
            [
                layout.allocSize(interpOrder, tv.EzCentering[0], tv.nbrCells[0]),
                layout.allocSize(interpOrder, tv.EzCentering[1], tv.nbrCells[1]),
                layout.allocSize(interpOrder, tv.EzCentering[2], tv.nbrCells[2]),
            ],
            dtype=np.float64,
        )
        ...
        dzBy = np.zeros(
            [
                layout.allocSize(interpOrder, tv.ByCentering[0], tv.nbrCells[0]),
                layout.allocSize(interpOrder, tv.ByCentering[1], tv.nbrCells[1]),
                layout.allocSizeDerived(interpOrder, tv.ByCentering[2], tv.nbrCells[2]),
            ],
            dtype=np.float64,
        )
        ...
        dzEz = np.zeros(
            [
                layout.allocSize(interpOrder, tv.EzCentering[0], tv.nbrCells[0]),
                layout.allocSize(interpOrder, tv.EzCentering[1], tv.nbrCells[1]),
                layout.allocSizeDerived(interpOrder, tv.EzCentering[2], tv.nbrCells[2]),
            ],
            dtype=np.float64,
        )
        ...
        dzBy[:, :, psi_p_Z : pei_p_Z + 1] = (
            By[:, :, psi_d_Z : pei_d_Z + 2] - By[:, :, psi_d_Z - 1 : pei_d_Z + 1]
        ) / tv.meshSize[2]
        dzEz[:, :, psi_p_Z : pei_p_Z + 1] = (
            Ez[:, :, psi_d_Z : pei_d_Z + 2] - Ez[:, :, psi_d_Z - 1 : pei_d_Z + 1]
        ) / tv.meshSize[2]
