...
diag_dir = "phare_outputs/shock_timevarying_inflow"


def inflow_density(x, t):
    # space-time callable: uniform in space, oscillates in time about
    # LEFT_INIT.rho. The inflow boundary must track this function at
    # runtime -- this is what the assertion below checks.
    return LEFT_INIT.rho * (1.0 + 0.1 * np.sin(2.0 * np.pi * t / final_time))
...
def assert_inflow_tracks_timevarying_density(diag_dir):
    """Proves at RUNTIME that the xlower inflow boundary tracked the
    time-varying `inflow_density(x, t)` space-time callable: for each
    dumped timestamp, the leftmost physical cell's density is close to
    the prescribed function of time (not the constant LEFT_INIT.rho),
    and the whole field stays finite.
    """
    if cpp.mpi_rank() != 0:
        return
    ...
    for t in timestamps:
        ...
        expected = inflow_density(0.0, t)
        assert np.isclose(rho_left, expected, rtol=5e-2), (
            f"inflow density at t={t} did not track time-varying function: "
            f"got {rho_left}, expected ~{expected}"
        )
        checked_any = True
...
class ShockTest(SimulatorTest):
    ...
    def test_run(self):
        ...
        print_case_info()
        assert_inflow_tracks_timevarying_density(diag_dir)
        # if cpp.mpi_rank() == 0:
