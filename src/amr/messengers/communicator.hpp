...
namespace amr
{
     ...
     */
    template<typename ResourcesManager>
    Communicator<Refiner> makeRefiner(VecFieldDescriptor const& source,
                                      VecFieldDescriptor const& destination,
                                      std::shared_ptr<ResourcesManager> const& rm,
                                      std::shared_ptr<SAMRAI::hier::RefineOperator> refineOp)
    {
        Communicator<Refiner> com;

        auto registerRefine = [&com, &rm, &refineOp](std::string src, std::string dst) {
        ...
        registerRefine(source.xName, destination.xName);
        registerRefine(source.yName, destination.yName);
        registerRefine(source.zName, destination.zName);
