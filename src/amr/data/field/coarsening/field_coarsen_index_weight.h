...
{
private:
    std::array<std::size_t, dimension> nbrFinePoints_(std::array<QtyCentering, dimension> centering,
                                                      SAMRAI::hier::IntVector const& ratio) const
    {
        ...
        {
            if (centering[iDir] == QtyCentering::primal && ratio(iDir) % 2 == 0)
            {
...
public:
    FieldCoarsenIndexesAndWeights(std::array<QtyCentering, dimension> centering,
    ...
    std::vector<double> const& weights(Direction dir) const
    {
