...
namespace PHARE
{
/** @brief This class gives an operator() that performs the coarsening of N fine nodes onto a given
 * coarse node
 *
 * A FieldCoarsener object is created each time the refine() method of the FieldCoarsenOperator is
 * called and its operator() is called for each coarse index.
 */
template<std::size_t dimension>
class FieldCoarsener
{
public:
    FieldCoarsener(std::array<QtyCentering, dimension> const& centering,
                    ...
                    Point<int, dimension> coarseIndex)
    {
        ...
        if constexpr (dimension == 1)
        {
            ...
            auto const& xWeights    = indexesAndWeights_.weights(Direction::X);
        ...
        else if constexpr (dimension == 2)
        {
            ...
            auto const& xWeights = indexesAndWeights_.weights(Direction::X);
            auto const& yWeights = indexesAndWeights_.weights(Direction::Y);
        ...
        else if constexpr (dimension == 3)
        {
            ...
            auto const& xWeights = indexesAndWeights_.weights(Direction::X);
            auto const& yWeights = indexesAndWeights_.weights(Direction::Y);
            auto const& zWeights = indexesAndWeights_.weights(Direction::Z);
