...
namespace solver
{
    ...
    public:
                                ...
                                bool isRegridding) override
        {
            ...
            {
                PHARE_LOG_START(5, "hybridLevelInitializer::initialize : root level init");
                model.initialize(level);
                messenger.fillRootGhosts(model, level, initDataTime);
                PHARE_LOG_STOP(5, "hybridLevelInitializer::initialize : root level init");
            }
            ...
            else
            {
                if (isRegridding)
                {
                    ...
                    PHARE_LOG_START(5, "hybridLevelInitializer::initialize : regriding block");
                    messenger.regrid(hierarchy, levelNumber, oldLevel, model, initDataTime);
                    PHARE_LOG_STOP(5, "hybridLevelInitializer::initialize : regriding block");
                }
                else
                {
                    PHARE_LOG_START(5, "hybridLevelInitializer::initialize : initlevel");
                    messenger.initLevel(model, level, initDataTime);
                    PHARE_LOG_STOP(5, "hybridLevelInitializer::initialize : initlevel");
                }
            }
