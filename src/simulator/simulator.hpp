...
// template<std::size_t _dimension, std::size_t _interp_order, std::size_t _nbRefinedPart = false>
// class Simulator : public ISimulator
// {
// public:
//     double startTime() override { return startTime_; }
...
//     using WorkingState_t = WorkingState<dimension, interp_order, nbRefinedPart>;
...
// double Simulator<dim, _interp, nbRefinedPart>::restarts_init(initializer::PHAREDict const& dict)
// {
//     rMan = restarts::RestartsManagerResolver::make_unique(*hierarchy_, *state_->hybridModel_,
//     dict);


//     PHARE_LOG_LINE_STR(dict.contains("restart_time"));
...
// void Simulator<dim, _interp, nbRefinedPart>::diagnostics_init(initializer::PHAREDict const& dict)
// {
//     dMan = PHARE::diagnostic::DiagnosticsManagerResolver::make_unique(*hierarchy_,
//                                                                       *state_->hybridModel_,
//                                                                       dict);
...
//     hybridModel_       = std::make_shared<HybridModel>(
//         dict["simulation"], std::make_shared<typename
//         HybridModel::resources_manager_type>(prefix));
//     hybridModel_->resourcesManager->registerResources(hybridModel_->state);
...
//     state->integrator_ = std::make_shared<typename State_t::Integrator>(
...
//     PHARE_LOG_LINE_STR(startTime_);
//     PHARE_LOG_LINE_STR(currentTime_);
...
//     catch (std::runtime_error const& e)
//     {
//         std::cerr << "runtime_error CAUGHT: " << e.what() << std::endl;
//         std::rethrow_exception(std::current_exception());
//     }
...
// std::unique_ptr<PHARE::ISimulator> getSimulator(std::shared_ptr<PHARE::amr::Hierarchy>&
// hierarchy);
...
// std::unique_ptr<Simulator> makeSimulator(std::shared_ptr<amr::Hierarchy> const& hierarchy)
// {
//     return std::make_unique<Simulator>(initializer::PHAREDictHandler::INSTANCE().dict(),
//     hierarchy);
// }
