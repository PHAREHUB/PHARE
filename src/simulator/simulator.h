...
auto patchHierarchyDatabase(PHARE::initializer::PHAREDict& amr)
{
    ...
    auto largestPatchSizeDB  = hierDB->putDatabase("largest_patch_size");
    auto smallestPatchSizeDB = hierDB->putDatabase("smallest_patch_size");
    auto ratioToCoarserDB    = hierDB->putDatabase("ratio_to_coarser");

    auto smallestPatchSize = amr["smallest_patch_size"].template to<int>();
    auto largestPatchSize  = amr["largest_patch_size"].template to<int>();
