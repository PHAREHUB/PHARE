#ifndef PHARE_DIAGNOSTIC_DETAIL_VTK_H5_VTKTYPES_HAX_TYPE_WRITER_HPP
#define PHARE_DIAGNOSTIC_DETAIL_VTK_H5_VTKTYPES_HAX_TYPE_WRITER_HPP
...
struct HierarchyData
{
    ...
    static void reset(auto& h5Writer) // called once per dump
    {
        ...
        data.flattened_all_level_boxes.resize(maxLevels);
        ...
        h5Writer.modelView().onLevels(
            [&](auto const& level) {
                ...
                auto const nranks = data.level_boxes_per_rank[ilvl].size();
                data.flattened_all_level_boxes[ilvl].resize(nranks);
                for (std::size_t r = 0; r < nranks; ++r)
                    data.flattened_all_level_boxes[ilvl][r]
                        = flatten_boxes(data.level_boxes_per_rank[ilvl][r]);
            ...
            [&](int const ilvl) { // ilvl does not exist currently
                ...
                data.flattened_all_level_boxes[ilvl].clear();
            },
    ...
    std::vector<std::vector<std::vector<std::array<int, dim * 2>>>> flattened_all_level_boxes;
...
public:
    ...
    void closeFile(DiagnosticProperties const& diagnostic) { fileData_.erase(diagnostic.quantity); }

    void reopenFile(DiagnosticProperties const& diagnostic)
    {
        fileData_.erase(diagnostic.quantity);
        fileData_.emplace(diagnostic.quantity, h5Writer_.makeFile(diagnostic, /*parallel=*/true));
    }
...
public:
    VTKFileWriter(DiagnosticProperties const& prop, H5TypeWriter<Writer>* const typewriter,
                  std::size_t const offset, int const ilvl);
...
private:
    ...
    std::size_t compute_offset(int const ilvl) const
    {
        if (!h5file.exist(level_data_path(ilvl)))
            return 0; // no write for missing/inactive file

        auto const& hier_data = HierData::INSTANCE();
        auto const new_size   = h5file.getDataSet(level_data_path(ilvl)).getDimensions()[0];
        auto offset = static_cast<std::size_t>(new_size - hier_data.level_data_size[ilvl]);
        for (int i = 0; i < core::mpi::rank(); ++i)
            offset += hier_data.level_rank_data_size[ilvl][i];
        return offset;
    }
...
H5TypeWriter<Writer>::VTKFileWriter::VTKFileWriter(DiagnosticProperties const& prop,
    ...
    , typewriter{tw}
    , h5file{tw->getOrCreateH5File(prop)}
    , data_offset{compute_offset(ilvl)}
{
}
...
void H5TypeWriter<Writer>::VTKFileInitializer::resize_boxes(int const ilvl)
{
    ...
    for (std::size_t r = 0; r < rank_boxes.size(); ++r)
    {
        if (rank_boxes[r].empty())
            continue;
        amrbox_ds.select({box_offset, 0}, {rank_boxes[r].size(), dimension * 2})
            .write(hier_data.flattened_all_level_boxes[ilvl][r]);
        box_offset += rank_boxes[r].size();
    }
}
...
#endif // PHARE_DIAGNOSTIC_DETAIL_VTK_H5_VTKTYPES_HAX_TYPE_WRITER_HPP
