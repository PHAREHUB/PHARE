...
void FluidDiagnosticWriter<H5Writer>::write(DiagnosticProperties& diagnostic)
{
    ...
    auto const write_quantity = [&](auto& layout, auto const&, auto const iLevel) {
        if (isActiveDiag(diagnostic, tree, "charge_density"))
            initializer.writeField(ions.chargeDensity(), layout);
        if (isActiveDiag(diagnostic, tree, "mass_density"))
            initializer.writeField(ions.massDensity(), layout);
        if (isActiveDiag(diagnostic, tree, "bulkVelocity"))
            initializer.template writeTensorField<1>(ions.velocity(), layout);
    };
    ...
    modelView.onLevels(
        [&](auto const& lvl) {
            ...
            if (isActiveDiag(diagnostic, tree, "charge_density"))
                initializer.initFieldFileLevel(ilvl, boxes);
            if (isActiveDiag(diagnostic, tree, "mass_density"))
                initializer.initFieldFileLevel(ilvl, boxes);
            if (isActiveDiag(diagnostic, tree, "bulkVelocity"))
