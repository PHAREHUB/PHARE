...
private:
    ...
    std::unordered_map<size_t, std::vector<std::string>> patchIDs_;
    ...
    */
    ...
    auto collectPatchAttributes
        = [&]([[maybe_unused]] GridLayout& gridLayout, std::string patchID, size_t iLevel) {
              if (!patchIDs_.count(iLevel))
                  patchIDs_.emplace(iLevel, std::vector<std::string>());

              patchIDs_.at(iLevel).emplace_back(patchID);
    ...
    {
        writers.at(diagnostic->type)->initDataSets(*diagnostic, patchIDs_, patchAttributes);
    }
}
