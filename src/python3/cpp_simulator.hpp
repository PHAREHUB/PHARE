...
/**
 * @brief Declare non-simulator-specific Python bindings for this configuration
 * 
 * Creates Python bindings for:
 * - DataWrangler: Access to simulation data on patches
 * - PatchLevel: Interface to data on a specific refinement level  
 * - Splitter: Particle refinement utilities
 * - split_pyarray_particles: Function to split particle arrays for refinement
 * 
 * @tparam Sim The simulator type for this configuration
 * @param m Pybind11 module to add bindings to
 */
...
void inline declare_etc(py::module& m)
{
    ...
    // Bind DataWrangler for accessing simulation data
...
/**
 * @brief Main entry point for creating Python bindings for a simulator configuration
 * 
 * This function is called from the PYBIND11_MODULE macro in cpp_simulator.cpp
 * to create all Python bindings for a specific simulator configuration.
 * 
 * Creates bindings for:
 * - Simulator class with configuration-specific static properties
 * - make_simulator factory function
 * - All supporting classes (DataWrangler, PatchLevel, Splitter, etc.)
 * 
 * @param m Pybind11 module (e.g., cpp_1_2_3) to populate with bindings
 */
void inline declare_macro_sim(py::module& m)
{
    using Sim = Simulator<SimOpts{PHARE_SIM_STR}>;

    // Create Simulator binding with static properties for this configuration
