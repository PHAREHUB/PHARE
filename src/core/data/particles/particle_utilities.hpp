...
auto positionAsPoint(std::array<int, dim> const& iCell_,    //
...
template<typename GridLayout, std ::size_t dim = GridLayout::dimension>
auto positionAsPoint(Particle<dim> const& particle, GridLayout const& layout)
{
    return positionAsPoint(particle.iCell(), particle.delta(), layout);
}

template<typename ParticleArray_t, typename GridLayout>
auto positionAsPoint(ParticleArray_t const& particles, std::size_t idx, GridLayout const& layout)
{
