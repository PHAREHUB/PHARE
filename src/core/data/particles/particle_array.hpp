...
#if 0 // TODO is that even needed ? if yes, needs to be consistent with cellmap
    iterator erase(iterator position)
    {
        return iterator{particles_.erase(position), *this, cellMap_};
    }
#endif
...
*/
    ...
    void export_particles(box_t const& box, ParticleArray<dim>& dest, Fn&& fn) const
    {
        ...
        cellMap_.export_to(box, particles_, dest.particles_, std::forward<Fn>(fn));
    }
    ...
    void export_particles(This& dest, Predicate&& pred) const
    {
        ...
        cellMap_.export_to(particles_, dest.particles_, std::forward<Predicate>(pred));
    }

    template<typename Cell>
    void change_icell(Cell const& oldCell, std::size_t particleIndex)
    {
