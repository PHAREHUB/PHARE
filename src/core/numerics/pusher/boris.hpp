...
private:
    ...
    auto advancePosition_(Particle const& partIn, Particle& partOut)
    {
        ...
        for (std::size_t iDim = 0; iDim < dim; ++iDim)
        {
            ...
            oldCell[iDim]       = partIn.iCell[iDim];
    ...
    void pushStep_(ParticleRange const& rangeIn, ParticleRange& rangeOut, PushStep step)
    {
             ...
             ++inIdx, ++outIdx)
        {
            ...
            auto oldCell = advancePosition_(inParticles[inIdx], outParticles[outIdx]);
            outParticles.change_icell(oldCell, outIdx);
        }
    }
