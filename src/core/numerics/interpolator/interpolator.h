 ...
 *
 *  \param[in] normalized position is the particle position in the cell normalized by grid spacing
 *  \param[in] startIndex first grid index where to interpolate the field
 *  \param[out] weights contains the nbrPointsSupport weights calculated
 *
...
public:
     ...
     * \param[in] startIndex is the first of the nbrPointsSupport indices where to interpolate the
     * field \param[in] weights are the nbrPointsSupport weights used for the interpolation
     */
...
public:
     ...
     * \param[in] startIndex is the first of the nbrPointsSupport indices where to interpolate the
     * field in both directions \param[in] weights are the nbrPointsSupport weights used for the
...
public:
     ...
     * \param[in] fieldCentering is the centering (dual or primal) of the field in each direction
     * \param[in] startIndex is the first of the nbrPointsSupport indices where to interpolate the
     * field in the 3 directions \param[in] weights are the nbrPointsSupport weights used for the
     * interpolation in the 3 directions
     */
    template<typename Field, typename Array1, typename Array2>
        ...
        auto indexAndWeightDual = [this](auto const& part) {
            for (auto iDim = 0u; iDim < GridLayout::dimension; ++iDim)
            {
                double normalizedPos
                    = part.iCell[iDim] + part.delta[iDim] + dualOffset(GridLayout::interp_order);
