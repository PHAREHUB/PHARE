#ifndef PHARE_CORE_NUMERICS_ION_UPDATER2_HPP
#define PHARE_CORE_NUMERICS_ION_UPDATER2_HPP
...
template<typename Ions>
void IonUpdater2<Ions>::updatePopulations(UpdaterMode const mode, auto& ions, auto&&... args)
{
    PHARE_LOG_SCOPE(3, "IonUpdater2::updatePopulations");
...
/**
 * @brief IonUpdater2<Ions>::updateAndDepositDomain_
   evolves moments from time n to n+1 without updating particles, which stay at time n
 */
template<typename Ions>
void IonUpdater2<Ions>::updateAndDepositDomain_(auto& ions, auto const& em, auto const& boxing,
                                                double const dt)
{
    bool constexpr copy_particle = true;

    PHARE_LOG_SCOPE(3, "IonUpdater2::updateAndDepositDomain_");
...
template<typename Ions>
void IonUpdater2<Ions>::updateAndDepositAll_( //
    auto& ions, auto const& em, auto const& boxing, double const dt)
{
    bool constexpr copy_particle = false;

    PHARE_LOG_SCOPE(3, "IonUpdater2::updateAndDepositAll_");
...
template<typename Ions>
void IonUpdater2<Ions>::Pusher::move_particle( //
...
template<typename Ions>
template<bool copy_particle>
void IonUpdater2<Ions>::Pusher::move_domain( //
    auto& pop, auto const& em, auto const& boxing, auto& interpolator) const
{
    ...
    patch_ghost.reserve(static_cast<std::size_t>(domain.size() / 100)); // do better
    ...
    for (std::size_t i = 0; i < domain.size(); ++i) // size might change on iteration!
    {
        ...
        {
            interpolator.particleToMesh(particle, rhoP, rhoC, F, layout);
            continue;
        }

        bool const isInDomainBox      = boxing.isInDomainBox(particle);
        bool const isInPatchGhostBox  = boxing.isInPatchGhostBox(particle);
        bool const should_interpolate = isInDomainBox or isInPatchGhostBox;
        ...
        if constexpr (!copy_particle)
        {
            ...
            {
                patch_ghost.push_back(particle);
                domain.swap_last_reduce_by_one(oldCell, i);
                --i; // redo current index as last is now i
            }
        }
...
template<typename Ions>
template<bool copy_particle>
void IonUpdater2<Ions>::Pusher::move_level_ghost( //
    auto& pop, auto const& em, auto const& boxing, auto& interpolator) const
{
    using Particle_t     = std::decay_t<decltype(pop.domainParticles()[0])>;
    using ParticleLoop_t = std::conditional_t<copy_particle, Particle_t, Particle_t&>;
