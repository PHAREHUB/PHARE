...
/**
 * @brief IonUpdater2<Ions>::update_copy
   evolves moments from time n to n+1 without updating particles, which stay at time n
 */
template<typename Ions>
void IonUpdater2<Ions>::update_copy(auto& ions, auto const& em, auto const& boxing,
                                                double const dt)
{
    bool constexpr copy_particle = true;

    PHARE_LOG_SCOPE(3, "IonUpdater2::update_copy");
