 ...
 *   - @c "eos"                  : string, e.g. @c "ideal_gas" — optional, defaults to ideal gas
 *   - @c "to_conservative_init/heat_capacity_ratio" : double, required for the ideal gas
 *
...
{
    auto const model = dict.contains("eos")
                           ? getThermoModelFromString(dict["eos"].template to<std::string>())
                           : ThermoModel::ideal_gas;
