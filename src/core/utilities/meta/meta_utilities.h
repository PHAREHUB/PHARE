...
namespace core
{
    ...
    auto makeAtRuntime(std::size_t dim, std::size_t interpOrder, Maker&& maker)
    {
        ...
        Ptr_t p;
        auto constexpr interp_orders = possibleInterpOrders();
        auto constexpr dimensions    = possibleDimensions();

        std::apply(
            [&interp_orders, &p, dim, interpOrder, &maker](auto&&... dimension) {
                (
                    [&]() {
                        auto& dim1 = dimension;
                        std::apply(
                            [dim1, &p, dim, interpOrder, &maker](auto&&... order) {
                                (
                                    [dim1, &p, dim, interpOrder, order, &maker]() {
                                        if (!p)
