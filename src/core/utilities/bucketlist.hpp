...
{
    if (indexes_.count(itemIndex))
    {
        auto const& index = indexes_[itemIndex];
        if (index.bucket_idx < bucket_idx)
            return index.pos < static_cast<typename BucketListIndex::Bindex>(bucket_size);
        else if (index.bucket_idx == bucket_idx)
            return index.pos < static_cast<typename BucketListIndex::Bindex>(curr);
        else
            return false;
    }
    return false;
...
void BucketList<bucket_size>::remove(std::size_t itemIndex)
{
    ...
    auto itemBucketIdx   = indexes_[itemIndex];
    auto lastIdxInBucket = lastIdx_();
    auto last_idx        = buckets_[lastIdxInBucket.bucket_idx][lastIdxInBucket.pos];

    std::swap(buckets_[itemBucketIdx.bucket_idx][itemBucketIdx.pos],
              buckets_[lastIdxInBucket.bucket_idx][lastIdxInBucket.pos]);
    std::swap(indexes_[last_idx], indexes_[itemIndex]);
    decrement_();
}
