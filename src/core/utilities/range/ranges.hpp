...
struct BalancedRangesDefaults
{
    ...
    auto static constexpr builder  = [](auto range, auto& el) -> auto { return range; };
...
{
    using Array   = std::decay_t<std::invoke_result_t<Accessor&, typename Container::value_type&>>;
    using Range_t = Range<typename Array::iterator>;
    using BuildRange_t = std::decay_t<
        std::invoke_result_t<Builder&, Range_t const, typename Container::value_type&>>;

    auto arrays = core::generate_from([&](auto& el) { return &accessor(el); }, container);
