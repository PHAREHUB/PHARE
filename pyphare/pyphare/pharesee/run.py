...
class Run:
    ...
    def _get_hierarchy(self, time, filename, hier=None):
        ...
        return hierarchy_from(
            h5_filename=os.path.join(self.path, filename), time=t, hier=hier
        )
    ...
    def GetVe(self, time, merged=False, interp="nearest"):
        J = self.GetJ(time, merged, interp)
        Ni = self.GetNi(time, merged, interp)
        Vi = self.GetVi(time, merged, interp)
        pass
