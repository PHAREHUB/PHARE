...
def _compute_copy_do(patch_data, name, λ):
    return patch_data.copy_as(λ(patch_data.dataset[:]), name=name)
...
def drop_patch_ghosts(patch, **kwargs):
    ...
    for name, pd in patch.patch_datas.items():
        pd_attrs.append(drop_patchdata_ghosts(pd))
    return tuple(pd_attrs)
...
def _compute_copy_op(patch, op, hinfo, other, reverse=False, key_map=None):
    ...
    return tuple(
        _compute_copy_do(pd, key_map[name], lambda ds: _(ds, data.get(name, key_map)))
        for name, pd in patch.patch_datas.items()
    )
...
def _compute_gaussian_filter_on_vectorfield(patch, sigma, hinfo):
    ...
    return (
        patch["x"].copy_as(dsx_, name="x"),
        patch["y"].copy_as(dsy_, name="y"),
        patch["z"].copy_as(dsz_, name="z"),
    )
...
def _compute_grad(patch, **kwargs):
    ...
    return (
        patch[ref_name].copy_as(ds_x, name="x"),
        patch[ref_name].copy_as(ds_y, name="y"),
        patch[ref_name].copy_as(ds_z, name="z"),
    )
