...
def interpolate_hierarchy(hier, box=None, quantity=None, interp="nearest"):
    ...
    if isinstance(box, (list, tuple)):
        raise TypeError(
            "interpolate_hierarchy takes a single box; call it once per box"
        )
    ...
    patch0 = deepcopy(hier.levels(time)[0].patches[0])
    patch0.layout.dl = dl
    patch0.layout.box = layout_box
    patch0.layout.origin = list(origin)
    nbrGhosts = next(iter(patch0.patch_datas.values())).ghosts_nbr
