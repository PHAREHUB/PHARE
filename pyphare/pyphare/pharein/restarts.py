...
def validate(sim):
    ...
    if "elapsed_timestamps" in restart_options:
        ...
        restart_options["elapsed_timestamps"] = [
            ...
            for ts in phare_utilities.np_array_ify(restart_options["elapsed_timestamps"])]

        if not np.all(np.diff(restart_options["elapsed_timestamps"]) >= 0):
            raise RuntimeError(f"Error: restart_options elapsed_timestamps not in ascending order)")
    ...
    if "timestamps" in restart_options:
        restart_options["timestamps"] = phare_utilities.np_array_ify(restart_options["timestamps"])
        ...
        if not np.all(np.diff(timestamps) >= 0):
            raise RuntimeError(f"Error: restart_options timestamps not in ascending order)")
        if not np.all(np.abs(timestamps / sim.time_step - np.rint(timestamps/sim.time_step) < 1e-9)):
            raise RuntimeError(f"Error: restart_options timestamps is inconsistent with simulation.time_step")
