...
def validate(sim):
    ...
    if not np.all(np.diff(timestamps) >= 0):
        raise RuntimeError("Error: restart timestamps not in ascending order)")
    if not np.all(np.abs(timestamps / sim.time_step - np.rint(timestamps/sim.time_step) < 1e-9)):
        raise RuntimeError("Error: restart timestamps is inconsistent with simulation.time_step")
