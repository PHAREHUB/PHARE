// https://www.particleincell.com/2016/hip-pic/
// https://www.particleincell.com/wp-content/uploads/2016/02/sheath-gpu.cu


/*Demo 1D sheath PIC simulation with HIP
...
int main()
{
    ...
    HIP_ERROR(hipGetDeviceProperties(&prop, 0));
    ...
    HIP_ERROR(hipMalloc((void**)&ions.part_gpu, NUM_IONS * sizeof(Particle)));
    ...
    HIP_ERROR(hipMalloc((void**)&electrons.part_gpu, NUM_ELECTRONS * sizeof(Particle)));
    ...
    HIP_ERROR(
        hipMemcpy(ions.part_gpu, ions.part, NUM_IONS * sizeof(Particle), hipMemcpyHostToDevice));
    HIP_ERROR(hipMemcpy(electrons.part_gpu, electrons.part, NUM_IONS * sizeof(Particle),
    ...
    delete nde;
    delete ndi;
    HIP_ERROR(hipFree(nde_gpu));
    HIP_ERROR(hipFree(ndi_gpu));
    HIP_ERROR(hipFree(ef_gpu));

    /*free particles*/
    delete ions.part;
    delete electrons.part;
    HIP_ERROR(hipFree(ions.part_gpu));
    HIP_ERROR(hipFree(electrons.part_gpu));

    printf("Time per time step: %.3g ms\n",
           1000 * (end - start) / (double)(CLOCKS_PER_SEC * NUM_TS));

    /* call hipDeviceReset for correct profiling data*/
